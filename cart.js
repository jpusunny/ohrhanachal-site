/* Ohr Hanachal — shared cart (localStorage + slide-out drawer).
   Include on every page: <script src="cart.js"></script> (defer not needed). */
(function(){
  const KEY = 'oh_cart';
  ...
  // ---- add: scrape the nearest product container ----
  window.addToCart = function(){
    const t = window.event && window.event.target;
    let title='Sefer', price=0, img='', qty=1;
    const card = t && t.closest('.card');
    if (card){
      title = card.querySelector('h3').textContent.trim();
      price = priceOf(card.querySelector('.price').textContent);
      const im = card.querySelector('img'); img = im ? im.src : '';
    } else {
      const pinfo = document.querySelector('.pinfo');
      if (pinfo){
        title = pinfo.querySelector('h1').textContent.trim();
        price = priceOf((document.getElementById('price')||{}).textContent || pinfo.querySelector('.now').textContent);
        const mi = document.getElementById('mainImg'); img = mi ? mi.src : '';
        const qe = document.getElementById('q'); qty = qe ? (parseInt(qe.textContent)||1) : 1;
      }
    }
    const cart = get();
    const ex = cart.find(i => i.title===title && i.price===price);
    if (ex) ex.qty += qty; else cart.push({title, price, img, qty});
    set(cart);
    openCart();
  };
  ...
  window.OHCart = {
    ...
    get, money, openCart, closeCart, clear:()=>set([])
  };
